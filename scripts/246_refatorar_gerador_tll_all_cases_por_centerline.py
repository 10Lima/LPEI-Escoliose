...
MAX_FILES = safe_int(os.environ.get("SPINAL_246_MAX_FILES", "0"))
OVERWRITE = os.environ.get("SPINAL_246_OVERWRITE", "0").strip() == "1"
SMOOTH_VALUES = [float(item) for item in os.environ.get("SPINAL_246_SMOOTH_VALUES", "0,30,60,120").split(",") if item.strip()]
MIN_POINT_DISTANCES = [float(item) for item in os.environ.get("SPINAL_246_MIN_POINT_DISTANCES", "8,15,35").split(",") if item.strip()]
MIN_NORM_SPANS = [float(item) for item in os.environ.get("SPINAL_246_MIN_NORM_SPANS", "0.04,0.12").split(",") if item.strip()]
MAX_EXPORT_PER_COMBO = safe_int(os.environ.get("SPINAL_246_MAX_EXPORT_PER_COMBO", "50"))
MAX_DIVERSE_PER_COMBO = safe_int(os.environ.get("SPINAL_246_MAX_DIVERSE_PER_COMBO", "25"))
MAX_EXPORT_PER_FILE = safe_int(os.environ.get("SPINAL_246_MAX_EXPORT_PER_FILE", "1200"))

TLL_WINDOWS = [
    ("tll_lower_035_100", 0.35, 1.00, "tll_historical_region"),
    ("tll_lower_025_100", 0.25, 1.00, "tll_extended_upper"),
    ("tll_global_000_100", 0.00, 1.00, "tll_global"),
]
...
def load_stack(centerline_subdir):
    ...
    s120 = load_module(
        f"script120_for_246_{centerline_subdir}",
...
def candidate_output_path(source_name):
    ...
    return OUTPUT_DIR / source_name / "candidates_valid" / f"tll_candidates{suffix}.csv"


def ensure_output_path(path):
    if path.exists() and not OVERWRITE:
        raise FileExistsError(f"Output ja existe: {path}. Usa SPINAL_246_OVERWRITE=1 para substituir.")
    path.parent.mkdir(parents=True, exist_ok=True)
...
def candidate_row(context, pair, local_rank, smooth, window_name, start, end, family, min_distance, min_span, reason):
    ...
    return {
        ...
        "target": TARGET,
        "source": "tll246_all_cases_from_196_159_175_logic",
        "source_group": "new",
        "method": window_name,
        "rank": 0,
        ...
        "base_tll": round_safe(base_value),
        "base_selection_score_tll": round_safe(context["base_score"]),
        "base_status_tll": context["base_status"],
        ...
        "gt_tll_diag": round_safe(gt_value),
...
def generate_for_case(s120, spline_diag, cobb, centerline_path, gt_by_file):
    ...
    for smooth in SMOOTH_VALUES:
        ...
        for window_name, start, end, family in TLL_WINDOWS:
...
def summarize_source(source_name, centerline_subdir, files, candidate_rows, case_rows, failures):
    ...
    oracle_le5 = sum(safe_float(row["oracle_tll_abs_error_diag"]) <= GOOD_LIMIT for row in case_rows)
    oracle_le10 = sum(safe_float(row["oracle_tll_abs_error_diag"]) <= WEAK_LIMIT for row in case_rows)
    ...
    return {
        ...
        "oracle_tll_le5_count": oracle_le5,
        "oracle_tll_le5_pct": round(float(oracle_le5 / total_cases * 100.0), 4) if total_cases else 0,
        "oracle_tll_le10_count": oracle_le10,
        "oracle_tll_le10_pct": round(float(oracle_le10 / total_cases * 100.0), 4) if total_cases else 0,
...
def run_source(source_name, centerline_subdir):
    ...
    for idx, path in enumerate(files, start=1):
        try:
            ...
            case_rows.append({
                ...
                "file_id": context["file_id"],
                "raw_file_id": context["raw_file_id"],
                "gt_tll_diag": round_safe(context["gt"]),
                "base_tll": round_safe(context["base"]),
                "candidate_count": len(rows),
                "oracle_tll": "" if best is None else best["cobb"],
                "oracle_tll_abs_error_diag": "" if best is None else best["candidate_abs_error_diag"],
                "oracle_tll_rank": "" if best is None else best["rank"],
            })
    ...
    write_csv(out_path.parent / "tll_case_summary.csv", case_rows)
    write_csv(out_path.parent / "tll_failures.csv", failures, ["source", "file_id", "raw_file_id", "error"])
    return summarize_source(source_name, centerline_subdir, files, candidate_rows, case_rows, failures)


def write_docs(summaries):
    write_csv(OUTPUT_DIR / "246_tll_coverage_summary.csv", summaries)
    write_csv(OUTPUT_DIR / "246_tll_oracle_summary.csv", summaries)
    lines = [
        "# Resumo 246",
        "",
        "## Estado",
        "",
        "- Gerador TL/L all-cases criado com janelas historicas e expandidas.",
    ...
    for row in summaries:
        lines.append(
            ...
            f"{row['candidate_count']} candidatos, oracle <=5 {row['oracle_tll_le5_pct']}%, "
            f"falhas {row['failure_count']}."
        )
    (OUTPUT_DIR / "246_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    (OUTPUT_DIR / "246_manifest.md").write_text(
        "# Manifest 246\n\nGerador TL/L all-cases por CENTERLINE_SUBDIR. Base: regiao historica TL/L, variantes expandidas e exportacao diversa sem GT na inferencia.\n",
        encoding="utf-8",
    )
...
def main():
    ...
    for row in summaries:
        print(f"{row['source']}: cobertura {row['covered_images']}/{row['expected_cases']} candidatos={row['candidate_count']} oracle<=5={row['oracle_tll_le5_pct']}% falhas={row['failure_count']}")
    print(f"Resumo: {OUTPUT_DIR / '246_summary.md'}")
