...
def build_audit_rows(image_rows, target_rows, oracle_rows):
    ...
    for image in image_rows:
        ...
        image_audit.append({
            ...
            "mae3": image["mae3"],
            "fail191": int(safe_float(image["mae3"]) > 5.0),
            "severe191": int(safe_float(image["mae3"]) > 8.0),
            ...
            "pt_source191": image["pt_source191"],
            "mt_source191": image["mt_source191"],
            "tll_source191": image["tll_source191"],
            "changed_targets191": image.get("changed_targets191", ""),
        ...
        for target in TARGETS:
            ...
            target_audit.append({
                ...
                "source191": target_row["source191"],
                "image_mae3": image["mae3"],
                "image_fail191": int(safe_float(image["mae3"]) > 5.0),
                "image_severe191": int(safe_float(image["mae3"]) > 8.0),
                ...
                "oracle_total_gain_vs_191": round_safe(current_error - oracle_total_error if np.isfinite(oracle_total_error) else np.nan),
                "oracle_shortlist_gain_vs_191": round_safe(current_error - oracle_short_error if np.isfinite(oracle_short_error) else np.nan),
...
def summarize_counts(image_rows, target_rows):
    rows = []
    fail_images = [row for row in image_rows if safe_int(row["fail191"]) == 1]
    severe_images = [row for row in image_rows if safe_int(row["severe191"]) == 1]
...
def priority_rows(image_rows, target_rows):
    failed = [row for row in target_rows if safe_int(row["target_failed_gt5"]) == 1]
    scored = []
    for row in failed:
        gain_total = safe_float(row["oracle_total_gain_vs_191"])
        gain_short = safe_float(row["oracle_shortlist_gain_vs_191"])
        score = 0.0
        score += max(gain_total if np.isfinite(gain_total) else 0.0, 0.0)
        score += 5.0 if safe_int(row["image_severe191"]) else 0.0
...
def write_summary(count_rows, priority):
    counts = {(row["metric"], row["group"]): safe_int(row["count"]) for row in count_rows}
    lines = [
        "# Diagnostico 192 falhas checkpoint191",
        "",
        "## Entradas",
        "",
    ...
    lines.append("- Usar esta auditoria antes de criar novos gates/selectors.")
    (OUTPUT_DIR / "diagnostico192_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main():
    ...
    priorities = priority_rows(image_audit, target_audit)

    write_csv(OUTPUT_DIR / "diagnostico192_image_rows.csv", image_audit, union_fieldnames(image_audit))
    write_csv(OUTPUT_DIR / "diagnostico192_target_rows.csv", target_audit, union_fieldnames(target_audit))
    write_csv(OUTPUT_DIR / "diagnostico192_count_summary.csv", count_rows, union_fieldnames(count_rows))
    write_csv(OUTPUT_DIR / "diagnostico192_priority_targets.csv", priorities, union_fieldnames(priorities))
    write_summary(count_rows, priorities)

    print("\n===== DIAGNOSTICO192 FALHAS CHECKPOINT191 =====")
    for row in count_rows:
        if row["metric"] in {
    ...
    print(f"\nResumo: {OUTPUT_DIR / 'diagnostico192_summary.md'}")
    print("===== CONCLUIDO =====")
